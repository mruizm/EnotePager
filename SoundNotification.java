...
/**
 * Created by rumarco on 23/12/2016.
 */
public class SoundNotification {

    private static final String TAG = "BPMP";
    private static SoundNotification sInstance;
    ...
    public static SoundNotification getInstance(Context context) {
        if (sInstance == null) {
            sInstance = new SoundNotification(context);
        }
        return sInstance;
    }
    ...
    private SoundNotification(Context context) {
        mContext = context;
    ...
    public void play() {
        if (!mAudioIsPlaying) {
            if (mPlayer == null) {
                mPlayer = MediaPlayer.create(mContext, R.raw.pager_sound);
